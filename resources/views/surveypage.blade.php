                ...
                @foreach ($survey->questions as $question)
                    ...
                    @if (count($question->readyanswers) > 0)
                        ...
                        <select name="{{ $question->id }}" id="{{ $question->id }}" class="block mt-1 w-full">
                            @foreach ($question->readyanswers as $readyanswer)
                                <option value="{{ $readyanswer->text_answer }}">{{ $readyanswer->text_answer }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    @else
                        <div>
                            ...
                            <label for="{{ $question->id }}">{{ $question->text_question }}</label>
                            <x-text-input id="{{ $question->id }}" class="block mt-1 w-full" type="text"
                                name="{{ $question->id }}" required autofocus autocomplete="name" />
